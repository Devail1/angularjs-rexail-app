const rexailApp = angular.module("rexail-app", [
  "ngRoute",
  "infinite-scroll",
  "storeModule",
  "cartModule",
...
rexailApp.constant(
  "IMG_BASE_URL",
  "https://s3.eu-central-1.amazonaws.com/images-il.rexail.com/"
);
  ...
  function fetchAppData() {
    $http
      ...
      .then(function (response) {
        ...
        $http
          .get(
            `https://test.rexail.co.il/client/public/store/catalog?s_jwe=${$rootScope.globalState.storeData.jsonWebEncryption}`
          )
          .then(function (response) {
            // Formatting data products by categories
            $rootScope.globalState.categoriesData = formatData(
              response.data.data
            );
            // Setting initial category
            $rootScope.globalState.selectedCategory =
              $rootScope.globalState.categoriesData[0];
          });
      });
  }
      ...
      array.reduce(
        (obj, current) => {
          ...
          let primaryQuantityUnit =
            !current.product.primaryQuantityUnit &&
...
rexailApp.service("cartActionsService", function ($rootScope) {
  ...
  service.onIncreaseProductQuantity = function (product) {
    if (
      ...
      !$rootScope.globalState.cartItems.includes(product) &&
        $rootScope.globalState.cartItems.unshift(product);
  ...
  service.onRemoveProduct = function (product) {
    ...
    $rootScope.globalState.cartItems = $rootScope.globalState.cartItems.filter(
  ...
  service.calculateTotal = function () {
    ...
    const sumWithInitial = $rootScope.globalState.cartItems.reduce(
      (totalSum, product) => totalSum + product["price"] * product.quantity,
      initialValue
    );
...
rexailApp.controller("productController", function (CURRENCY_SIGN) {
  ...
  ctrl.onUnitTypeChange = function (product, newQuantityUnit) {
    ...
    if (
      product.primaryQuantityUnit.estimatedUnitWeight <
      newQuantityUnit.estimatedUnitWeight
    ) {
      product.price = (
        product.price * newQuantityUnit.estimatedUnitWeight
      ).toFixed(2);
      if (product.originalPrice) {
        product.originalPrice = (
          product.originalPrice * newQuantityUnit.estimatedUnitWeight
        ).toFixed(2);
      }
    } else if (
      product.primaryQuantityUnit.estimatedUnitWeight !==
      ...
      product.price = (
        product.price / product.primaryQuantityUnit.estimatedUnitWeight
      ).toFixed(2);
      if (product.originalPrice) {
        product.originalPrice = (
          product.originalPrice /
  ...
  return {
    ...
    link: function (ngModelController) {
      ngModelController.$parent.$parent.productForm = {
        $valid: function () {
          return (
            $rootScope.globalState.cartItems.filter(
              (product) => product.commentType
            ).length ===
            $rootScope.globalState.cartItems.filter(
              (product) => product.commentType && product.comment
            ).length
          );
        },
      };
